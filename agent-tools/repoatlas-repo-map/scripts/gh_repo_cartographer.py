...
def github_repo_name_from_url(value: str) -> str | None:
    parsed = urlparse(value.strip())
    if parsed.netloc.lower() != "github.com":
        return None
...
def discover_repo_address_files(values: list[str]) -> list[Path]:
    raw_files: list[str] = []
    env_files = os.environ.get("GH_REPO_CARTOGRAPHER_REPO_ADDRESS_FILE")
    if env_files:
        raw_files.extend([part for part in env_files.split(os.pathsep) if part.strip()])
    raw_files.extend(values)
    ...
    for raw in raw_files:
        ...
        try:
            ...
            resolved = path.absolute()
        if resolved.exists() and resolved.is_file() and resolved not in files:
...
def list_remote_repos_from_address_files(paths: list[Path]) -> list[dict[str, Any]]:
    remotes: list[dict[str, Any]] = []
    seen: set[str] = set()
    for path in paths:
        for raw in path.read_text(encoding="utf-8", errors="replace").splitlines():
            match = GITHUB_REPO_URL_RE.search(raw)
            if not match:
                continue
            url = match.group(0).rstrip("/")
            name_with_owner = github_repo_name_from_url(url)
            key = normalize_repo_key(name_with_owner or url)
            if not name_with_owner or not key or key in seen:
                continue
            seen.add(key)
            owner = name_with_owner.split("/", 1)[0]
            flags_match = re.search(r"\[([^\]]+)\]", raw)
            flags = {flag.strip().lower() for flag in (flags_match.group(1).split(",") if flags_match else [])}
            remotes.append(
                {
                    ...
                    "isArchived": "archived" in flags,
                    "isFork": "fork" in flags,
...
def merge_remote_repo_lists(*repo_lists: list[dict[str, Any]]) -> list[dict[str, Any]]:
    merged: list[dict[str, Any]] = []
    seen: set[str] = set()
    for repo_list in repo_lists:
        for repo in repo_list:
            key = normalize_repo_key(repo.get("repoKey") or repo.get("nameWithOwner") or repo.get("url"))
            if not key or key in seen:
                continue
            seen.add(key)
            repo["repoKey"] = key
            merged.append(repo)
    return sorted(merged, key=lambda item: item["nameWithOwner"].lower())
...
def render_repo_address_text(inventory: dict[str, Any]) -> str:
    rows = inventory["rows"]
    owners = sorted({(row["remote"].get("nameWithOwner") or "").split("/", 1)[0] for row in rows if row["remote"].get("nameWithOwner")})
    lines = [
        "# GitHub repository addresses with local paths",
        f"# Generated: {datetime.now(timezone.utc).astimezone().isoformat(timespec='seconds')}",
        f"# Total remote repositories: {inventory['remoteCount']}",
        f"# Remote repositories with local matches: {inventory['matchedRemoteCount']}",
        "# Format: <github-url>    # <owner/repo> [flags] local=<path1> ; <path2> | local=missing",
        "",
    ]
    ...
    for owner in owners:
        owner_rows = [
            row
            for row in rows
            if (row["remote"].get("nameWithOwner") or "").lower().startswith(f"{owner.lower()}/")
        ]
        lines.append(f"## {owner} ({len(owner_rows)})")
        for row in owner_rows:
            remote = row["remote"]
            name = remote.get("nameWithOwner") or ""
            url = remote.get("url") or f"https://github.com/{name}"
            flags = ["private" if remote.get("isPrivate") else "public"]
            if remote.get("isFork"):
                flags.append("fork")
            if remote.get("isArchived"):
                flags.append("archived")
            local_paths = [
                local.get("path")
                for local in row.get("localMatches", [])
                if local.get("path")
            ]
            local_text = " ; ".join(local_paths) if local_paths else "missing"
            lines.append(f"{url}    # {name} [{', '.join(flags)}] local={local_text}")
        lines.append("")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Text file containing GitHub repository URLs. Repeatable. When used without --account, skips live repo enumeration.",
    )
...
def main() -> int:
    ...
    repo_address_files = discover_repo_address_files(args.repo_address_file)
    accounts = unique(args.account or ([] if repo_address_files else parse_router_accounts()))
    if not accounts and not repo_address_files:
        print(
            "error: no managed GitHub accounts or repo address files found. Pass --account, --repo-address-file, or configure gh-account-router.",
            file=sys.stderr,
        )
        return 2
    ...
    live_remote_repos = list_remote_repos(accounts) if accounts else []
    cached_remote_repos = list_remote_repos_from_address_files(repo_address_files) if repo_address_files else []
    remote_repos = merge_remote_repo_lists(live_remote_repos, cached_remote_repos)
    ...
    if args.repo_address_output:
        Path(args.repo_address_output).write_text(render_repo_address_text(inventory), encoding="utf-8")
